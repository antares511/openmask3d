...
import os
from openmask3d.data.load import (
    Camera,
    InstanceMasks3D,
    Images,
    PointCloud,
...
class PointProjector:
    ...
    def get_visible_points_view(self):
        ...
        for i, idx in tqdm(enumerate(indices)):  # for each view
            ...
            projected_points[i][mask] = np.column_stack(
                [
                    [
            ...
            visibility_mask = (
                np.abs(
                    sensor_depth[pi[1][inside_mask], pi[0][inside_mask]]
    ...
    def get_visible_points_in_view_in_mask(self):
        ...
        visible_points_view, projected_points, resolution = (
            self.get_visible_points_view()
        )
        visible_points_in_view_in_mask = np.zeros(
            (num_view, masks.num_masks, resolution[0], resolution[1]), dtype=bool
        )
        ...
        for i in tqdm(range(num_view)):
            for j in range(masks.num_masks):
                ...
                if len(proj_points) != 0:
                    visible_points_in_view_in_mask[i][j][
                        proj_points[:, 1], proj_points[:, 0]
                    ] = True
    ...
    def get_top_k_indices_per_mask(self, k):
        num_points_in_view_in_mask = self.visible_points_in_view_in_mask.sum(
            axis=2
        ).sum(axis=2)
                ...
                if point_coords.shape[0] > 0:
                    ...
                    for level in range(num_levels):
                        ...
                        if save_crops:
                            cropped_img.save(
                                os.path.join(
                                    out_folder, f"crop{mask}_{view}_{level}.png"
                                )
                            )
