...
def main(ctx: DictConfig):
    ...
    assert os.path.exists(
        ctx.data.masks.masks_path
    ), f"Path to masks does not exist: {ctx.data.masks.masks_path} - first run compute_masks_single_scene.sh!"
    masks = InstanceMasks3D(ctx.data.masks.masks_path)
    print(f"[INFO] Masks loaded. {masks.num_masks} masks found.")

    # 2. Load the camera configurations
    # camera = Camera(
    #     intrinsic_path=ctx.data.camera.intrinsic_path,
    ...
    # )
    camera = hydra.utils.instantiate(ctx.data.camera)
    print("[INFO] Camera configurations loaded.")

    # 3. Load the images
    # indices = np.arange(
    #     0,
    #     get_number_of_images(ctx.data.camera.poses_path),
    #     step=ctx.openmask3d.frequency,
    # )
    # images = Images(
    #     images_path=ctx.data.images.images_path,
