...
class Camera:
    ...
    def get_depth_paths(self):
        path_str = str(Path(self.data_path) / self.scene / "depth" / "*.png")
        depth_paths = natsorted(glob.glob(path_str))
        print("Found " + str(len(depth_paths)) + " depth images")
        return depth_paths
    ...
    def get_se3_poses(self):
        ...
        pose_path = str(Path(self.data_path) / self.scene / "pose" / "*.txt")
        ...
        for path in pose_path:
            with open(path, "r") as file:
                ...
                pose = invert_se3(transformation_matrix)
                se3_poses.append(pose[:3, :])

        print("Found " + str(len(se3_poses)) + " poses")
        return se3_poses
...
class Images:
    ...
    def get_rgb_paths(self):
        path_str = str(Path(self.data_path) / self.scene / "rgb" / "*.png")
        rgb_paths = natsorted(glob.glob(path_str))
        print("Found " + str(len(rgb_paths)) + " images")
        return rgb_paths
